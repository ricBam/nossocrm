  ...
  return (
    <>
      ...
      >
        ...
        <td className="px-3 md:px-6 py-3 font-bold text-slate-900 dark:text-white">{deal.title}</td>
        <td className="px-3 md:px-6 py-3 text-slate-600 dark:text-slate-300">{deal.companyName}</td>
        ...
        <td className="px-3 md:px-6 py-3 font-mono text-slate-700 dark:text-slate-200">
        ...
        {customFieldDefinitions.map((field) => (
          <td key={field.id} className="px-3 md:px-6 py-3 text-right text-slate-600 dark:text-slate-300 text-sm">
      ...
      <div className="h-full overflow-auto">
        <table className="w-full text-left text-sm border-collapse">
          <thead className="bg-slate-50/80 dark:bg-white/5 border-b border-slate-200 dark:border-white/5 sticky top-0 z-10 backdrop-blur-sm">
            <tr>
              <th className="px-3 md:px-6 py-3 font-bold text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider w-10"></th>
              <th className="px-3 md:px-6 py-3 font-bold text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                Negócio
              </th>
              <th className="px-3 md:px-6 py-3 font-bold text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                Empresa
              ...
              {customFieldDefinitions.map(field => (
                <th
                  key={field.id}
                  className="px-3 md:px-6 py-3 font-bold text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider text-right"
                >
                  {field.label}
                </th>
              ))}
