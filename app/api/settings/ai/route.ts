...
const UpdateOrgAISettingsSchema = z
  .object({
    ...
    aiProvider: z.enum(['google', 'openrouter']).optional(),
...
export async function GET() {
  ...
  const { data: orgSettings, error: orgError } = await supabase
    ...
    .select('ai_enabled, ai_provider, ai_model, ai_google_key, ai_openrouter_key, telegram_bot_token, telegram_chat_id')
  ...
  const aiProvider: Provider = orgSettings?.ai_provider === 'openrouter' ? 'openrouter' : 'google';
  ...
  const baseResponse = {
    aiEnabled,
    aiProvider,
    aiModel: orgSettings?.ai_model || AI_DEFAULT_MODELS[aiProvider],
  ...
  if (profile.role !== 'admin') {
    return json({ ...baseResponse, aiGoogleKey: '', aiOpenrouterKey: '' });
  }

  return json({
    ...baseResponse,
    aiGoogleKey: maskKey(orgSettings?.ai_google_key),
    aiOpenrouterKey: maskKey(orgSettings?.ai_openrouter_key),
  });
}
