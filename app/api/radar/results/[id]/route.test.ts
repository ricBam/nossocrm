...
// `then` fica em `vi.fn()` próprio (não `deals.delete`/`results.delete`) porque é
...
const deals = { delete: vi.fn(), update: vi.fn(), eq: vi.fn(), resolve: vi.fn() };
...
function fakeSupabase(opts: {
    ...
    dealDeleteFails?: boolean;
}) {
    const dealsBuilder: Record<string, unknown> = {
        delete: () => { deals.delete(); return dealsBuilder; },
        ...
        then: (r: (v: { error: { message: string } | null }) => unknown) => {
            const result = { error: opts.dealDeleteFails ? { message: 'RLS negou (teste)' } : null };
            ...
            if (name === 'profiles') {
                return {
                    select: () => ({
                        eq: () => ({
                            maybeSingle: async () => ({
                                data: { organization_id: ORG, role: opts.role ?? 'admin' },
                                error: null,
                            }),
                        }),
                    }),
...
describe('DELETE /api/radar/results/:id', () => {
    ...
    it('apaga a linha do Radar quando não há deal salvo', async () => {
        ...
        expect(deals.delete).not.toHaveBeenCalled();
    });

    it('apaga o deal DE VERDADE — nunca só marca deleted_at', async () => {
        ...
        // Nada nesta aplicação filtra `deals.deleted_at` — `dealsService.getAll`,
        // fonte do DEALS_VIEW_KEY que o Kanban renderiza, não menciona a coluna.
        // Um soft-delete aqui deixaria o card visível no funil com a evidência do
        // Radar já destruída. Só o hard delete faz o card sumir de fato.
        expect(deals.delete).toHaveBeenCalled();
        expect(deals.update).not.toHaveBeenCalled();
        expect(results.delete).toHaveBeenCalled();

        // A garantia de ordem é o motivo desta rota existir: se o delete do
        // deal e o delete do Radar fossem invertidos, um delete que falhasse
    ...
    it('responde 403 e não apaga nada quando o usuário não é admin', async () => {
        createClient.mockResolvedValue(
            fakeSupabase({ role: 'vendedor', row: { id: RESULT_ID, saved_deal_id: 'deal_1' } })
        );
        const res = await DELETE(req(), ctx);
    ...
    it('não apaga a linha do Radar e responde 500 quando o delete do deal falha', async () => {
        createClient.mockResolvedValue(
            fakeSupabase({ row: { id: RESULT_ID, saved_deal_id: 'deal_1' }, dealDeleteFails: true })
        );
        const res = await DELETE(req(), ctx);
        const json = await res.json();
...
describe('PATCH /api/radar/results/:id', () => {
    ...
    it('responde 403 e não grava nada quando o usuário não é admin', async () => {
        createClient.mockResolvedValue(fakeSupabase({ role: 'vendedor', row: { id: RESULT_ID, saved_deal_id: null } }));
        const res = await PATCH(patchReq({ dealId: 'deal_9' }), ctx);
        expect(res.status).toBe(403);
        expect(results.update).not.toHaveBeenCalled();
    });
